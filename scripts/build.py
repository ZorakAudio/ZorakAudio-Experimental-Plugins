...
def _c_string_literal(text: str) -> str:
    return "".join(_c_escape_utf8_units(text))


def write_jsfx_plugin_paths_header(cmake_build: Path, plugin_root: Path) -> Path:
    plugin_root = plugin_root.resolve()
    resource_dir = (plugin_root / "resource").resolve()

    source_root_text = plugin_root.as_posix()
    resource_dir_text = resource_dir.as_posix() if resource_dir.exists() else ""

    out_header = cmake_build / "JSFXPluginPaths.h"
    out_header.write_text(
        "".join(
            [
                "// Auto-generated by build.py (JSFX plugin source/resource paths)\n",
                "#pragma once\n",
                f'#define ZA_JSFX_SOURCE_ROOT "{_c_string_literal(source_root_text)}"\n',
                f'#define ZA_JSFX_RESOURCE_DIR "{_c_string_literal(resource_dir_text)}"\n',
            ]
        ),
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    for spec in selected:
        ...
        if spec.plugin_type == "jsfx":
            write_jsfx_plugin_paths_header(cmake_build, spec.root_dir)
        ...
        resource_dir = spec.root_dir / "resource"
        if resource_dir.is_dir():
            for artifact in vst3s:
                dst = install_vst3_dir / f"{artifact.stem}.resources"
                if dst.exists():
                    shutil.rmtree(dst)
            ...
            if enable_clap:
                for artifact in claps:
                    dst = install_clap_dir / f"{artifact.stem}.resources"
                    if dst.exists():
                        shutil.rmtree(dst)
                    shutil.copytree(resource_dir, dst)
