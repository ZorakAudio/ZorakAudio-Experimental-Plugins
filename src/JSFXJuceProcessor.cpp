...
#if !defined(DSPJSFX_RUNTIME_STATE_ABI) || DSPJSFX_RUNTIME_STATE_ABI != 2
 #error "Regenerate JSFXDSP.h and its AOT object with the patched dsp_jsfx_aot.py (runtime state ABI 2)."
#endif
...
static uint64_t jsfxSliderMask (double value) noexcept
{
    ...
    if (rounded < 0.0 && rounded >= -9223372036854775808.0)
        return static_cast<uint64_t> (static_cast<int64_t> (rounded));
    return 0;
}
...
extern "C" int jsfx_sliderchange (DSPJSFX_State* st, double sliderMask)
{
    ...
    const uint64_t mask = jsfxSliderMask (sliderMask);
    if (mask == 0)
        return 0;
...
public:
    ...
    {
        const uint64_t changeMask = (uint64_t) st.pendingSliderChangeMask;
        const uint64_t automateMask = (uint64_t) st.pendingSliderAutomateMask;
        const uint64_t automateEndMask = (uint64_t) st.pendingSliderAutomateEndMask;
        const uint64_t applyMask = changeMask | automateMask | automateEndMask;
