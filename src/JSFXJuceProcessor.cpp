...
#if !defined(DSPJSFX_RUNTIME_STATE_ABI) || DSPJSFX_RUNTIME_STATE_ABI != 3
 #error "Regenerate JSFXDSP.h and its AOT object with the patched dsp_jsfx_aot.py (runtime state ABI 3)."
#endif
...
static std::vector<JsfxSliderDecl> parseJsfxSliderDecls (const char* jsfxText, juce::String* outHelpText = nullptr)
{
    ...
    while (start < text.size())
    {
        ...
        if (sliderN < 1 || sliderN > DSPJSFX_MAX_SLIDERS)
            continue;
...
static jsfx_gfx::SliderMask jsfxSliderMask (double value, int directSliderIndex) noexcept
{
    ...
    if (directSliderIndex >= 0 && directSliderIndex < DSPJSFX_MAX_SLIDERS)
    {
        mask.set (directSliderIndex);
        return mask;
    }

    // Numeric mask arguments are REAPER's legacy low-64-bit form. Direct
    // sliderN arguments carry an explicit index above, so slider65..256 remain
    // lossless even though EEL numeric values cannot encode 256 independent bits.
    if (! std::isfinite (value))
        return mask;

    const double rounded = std::round (value);
    if (rounded >= 0.0 && rounded < 18446744073709551616.0) // 2^64, exclusive
        mask.words[0] = static_cast<uint64_t> (rounded);
    else if (rounded < 0.0 && rounded >= -9223372036854775808.0)
        mask.words[0] = static_cast<uint64_t> (static_cast<int64_t> (rounded));
    return mask;
}
...
static jsfx_gfx::SliderMask loadSliderMaskWords (const uint64_t* words) noexcept
{
    ...
    if (words != nullptr)
        for (int word = 0; word < DSPJSFX_SLIDER_MASK_WORDS; ++word)
            out.words[(size_t) word] = words[word];
    return out;
}
...
public:
    ...
    void processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages) override
    {
        struct SilenceOnMemoryFault
        {
            ...
            {
                if (state.memoryFault != 0) { audio.clear(); midi.clear(); }
            }
        } silenceOnMemoryFault { st, buffer, midiMessages };
    ...
    void consumeDspSliderChanges()
    {
        ...
        for (int i = 0; i < DSPJSFX_MAX_SLIDERS; ++i)
        {
            ...
            internalSliderPendingMask.set (i);
        ...
        for (size_t i = 0; i < DSPJSFX_MAX_SLIDERS; ++i)
        {
            ...
            if (internalSliderPendingMask.test ((int) i))
            {
                ...
                if (sliderValuesEquivalent (info, hostVal, shadowVal))
                    internalSliderPendingMask.reset ((int) i);
                else
                    newVal = shadowVal;
            }
...
void updateGfxSnapshotIfNeeded (int numSamples)
{
    ...
    std::memcpy (b.sliders.data(), st.sliders, sizeof (double) * DSPJSFX_MAX_SLIDERS);
...
std::array<GfxSnapshot, 3> gfxSnaps {};
    ...
    SliderMask internalSliderPendingMask {};
};
